...
<a href="index.php"><label for="op1">Realizar otra denuncia</label></a>
...
echo ' 
<form id="uploadForm" action="index.php?action=denunciarahora" method="post" enctype="multipart/form-data">
<div class="row">              
...
Agregar Imagen<input id="filedenuncia" type="file" multiple accept="image/jpeg, image/png" name="image_guatesegura"  style="display: none;">
</label>           
</div>
<div class="col-lg-4 col-md-4 col-sm-4 ">
...
<input type="hidden" name="tipodelito" id="tipodelito" value="1">
</div>
...
?>
    ...
    <script type="application/javascript">
      $(document).ready(function () {
		$("#alertButton").on('click touchstart', function(e){
			
		});
		  
        $("#btndenunciar").on('click touchstart',function(e){
			e.preventDefault();			
			if (navigator.geolocation) {
			navigator.geolocation.getCurrentPosition(function(){
				if($('#descripcion').val()=="")
			{
				showMessage('Ingrese una descripci&oacute;n para la denuncia a ingresar.');	
			}else
			{
				if($('#filedenuncia').val()=="")
				{
					showMessage('Adjunte una imagen para la denuncia a ingresar.');	
				}else
				{
					$('#myModal').modal('show');
	  ...
	  function showMessage(content,header){
			if(header){
      ...
      function showError(error) {
        switch (error.code) {
          case error.PERMISSION_DENIED:
			...
			$("#infoAlert").html("Permiso de obtener ubicación por geolocalización denegado, favor permitir acceso a ubicación para poder realizar la denuncia.");
	...
	<div class="modal fade" id="infoModal" role="dialog">
		...
		<div class="modal-header">
			...
			<h5 class="modal-title" id="infoTitle">Mensaje Informativo
			</h5>
		</div>
		<div class="modal-body">
			<p ><b id="infoContent">Some text in the modal.</b>
			</p>
		</div>
